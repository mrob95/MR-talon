...
def direction(m):
  try:
    mod = direction_modifiers[m["basic_keys.direction_modifiers"]]
  except KeyError:
    mod = ""
  try:
    dire = directions[m["basic_keys.directions"]]
  except KeyError:
    dire = "left"
...
ctx.keymap(
  {
    "{basic_keys.alphabet}": lambda m: press(alphabet[m["basic_keys.alphabet"]]),
    "big {basic_keys.alphabet}": lambda m: press(alphabet[m["basic_keys.alphabet"]].upper()),
    ...
    "{basic_keys.punctuation}": lambda m: Key(punctuation[m["basic_keys.punctuation"]])(None),
    "long {basic_keys.punctuation2}": [" ", lambda m: Key(punctuation[m["basic_keys.punctuation2"]])(None), " "],
    "{basic_keys.simple_keys} [{basic_keys.repeat}]": repeated_action(lambda m: Key(simple_keys[m["basic_keys.simple_keys"]])(None)),
    "{basic_keys.simple_keys_norepeat}": lambda m: press(simple_keys_norepeat[m["basic_keys.simple_keys_norepeat"]]),
